...
function getreservation(){
    ...
    foreach ($result as $record) {
        ...
        $reservation_day[date('Y-m-d', $day_record)]= true;
    }
    //日付の古い順に並び替え
...
function reservation($date,$reservation_day){
...
for($day = 1; $day <= $day_count; $day++, $youbi++){
    ...
    $reservation = reservation(date("Y-m-d",strtotime($date)),$reservation_day);
    if($today == $date){
    ...
    } elseif(reservation(date("Y-m-d",strtotime($date)),$reservation_day)){
